...
export default function EditCategoryListPage({
    ...
    function addCate(li) {
        ...
        //삭제할 카테고리와 같은 카테고리명을 만들려고 했을 때 거부
    ...
    function handleSubmit(event) {
        ...
        // 노래들의 카테고리만 추출; 카테고리는 존재하는데 노래는 없는 경우를 위함
        const categoriesWithSongs = [...new Set(editSongList.map(list => list.category))];
        //categories에서 editedName만 추출
        const editedCate = categories.map(cate => cate.editedName.trim());
        //기타 카테고리 추가여부 확인할 변수
        let shouldAddEtc = false;
        //기타카테고리 삭제 가능한지 여부(기타에 노래가 존재하면 제거할 수 없음.)
        const canDeleteEtc =
            categoriesWithSongs.includes("기타") && //기타카테고리를 가진 노래가 존재하는데
            editedCate.includes("기타removeMode")  // 기타카테고리를 삭제하려고 함

        //삭제할 카테고리들이 제거된 카테고리
        let filteredCate = editedCate.filter(cate => cate.indexOf("removeMode") === -1);
            ...
            if (cate.name !== cate.editedName) {
                editSongList = editSongList.map(song => {
                    ...
                    if (cate.editedName.indexOf("removeMode") === -1) {
                        ...
                        if (!shouldAddEtc) shouldAddEtc = true;
        ...
        if (shouldAddEtc && !filteredCate.includes("기타")) {
            filteredCate.push("기타");
        }

        setFilterList(filteredCate);
            ...
            .map((li, n) => (
                ...
                >
                    ...
                    <div className="w-fit flex flex-col justify-center gap-1 items-center pr-2 duration-500">
                        ...
                        <button
                            type="button"
                            disabled={li.editedName === "기타" || n === categories.length - 1}
                            onClick={() => changeOrder(n, 1)}
                            className="h-3 leading-3 disabled:text-[#f2f2f2] disabled:opacity-50 hover:text-[#00FFC2] duration-300"
                        >
                            ▽
    ...
    return (
        <>
            ...
            >
                <div className="w-full max-w-[22rem] mx-2 relative bg-[#1c1b1b] border border-[#f2f2f2] z-100 p-4 flex flex-col items-center">
                    ...
                    <div className="w-full flex justify-between items-center border border-t-0 px-2 py-2 gap-2">
                        <input
                            className="w-full bg-[#1c1b1b] px-2 py-1 border rounded-md outline-none"
                            value={query}
                            spellCheck="false"
                            onChange={(e) => {
